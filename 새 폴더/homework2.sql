...
CREATE TABLE `article`(id INT(50),
regDate DATETIME,
title VARCHAR(50),
`body` VARCHAR(50)
);
...
ALTER TABLE `article` MODIFY `id` INT(50) NOT NULL;
...
ALTER TABLE `article`MODIFY `id` INT(50) AUTO_INCREMENT;
# article 테이블 구조확인(desc)
DESC `article`
# 나머지 칼럼 모두에도 not null을 적용해주세요. 
ALTER TABLE `article`MODIFY `regDate` DATETIME NOT NULL;
ALTER TABLE `article`MODIFY `title` VARCHAR(50) NOT NULL;
ALTER TABLE `article`MODIFY `body` VARCHAR(50) NOT NULL;
# article 테이블 구조확인(desc)
DESC `article`
# id 칼럼에 UNSIGNED 속성을 추가하세요.
ALTER TABLE `article`MODIFY `id` INT(50) UNSIGNED NOT NULL AUTO_INCREMENT 
# 작성자(writer) 칼럼을 title 칼럼 다음에 추가해주세요.
ALTER TABLE `article` ADD COLUMN `writer` VARCHAR(50) AFTER title
...
ALTER TABLE `article` MODIFY `nickname` VARCHAR(50) NOT NULL AFTER `body`
# hit 조회수 칼럼 추가 한 후 삭제해주세요.
ALTER TABLE `article` ADD  `hit` INT(200) NOT NULL
